...
# 語法
# page.locator("{CSS_selector}").xxx()

# 遇到雙 class 時，記得兩個 class 最前面都要加上.
# . 前面可以填上標籤名稱，也可不填
# 空隔代表階級關係，有階級才加，例如底下兩個
...
# locator 對於 click、fill 等功能自帶等待效果
# 但若只是純等待，那需要額外加上.wait_for()
# 建議是直接寫 wait_for_selector() 或其他 wait_for_xxx() 家族的就好
# 也可用以下方式
# page.locator("text=訂單管理").inner_text().wait_for() # 取畫面上可見的文字
# page.locator("text=訂單管理").text_content().wait_for() # 取所有文字 (包含隱藏的) 
# page.get_by_text("訂單管理").wait_for()
# ===============================================================================
...
def login():
    try :
        with sync_playwright() as p:
            ...
            page.goto(r"https://accounts.shopee.tw/seller/login?next=https%3A%2F%2Fseller.shopee.tw%2F")
            page.wait_for_selector("text=訂單管理", state="visible") # state 可以不寫
            context.storage_state(path="shopee_state.json")
            browser.close()
    except:
        raise RuntimeError("Some unexpected error ocurred ")        


def auto_launch(item:ProductData):
    try:
        with sync_playwright() as p:
            browser = p.chromium.launch(headless=False, channel="chrome")
